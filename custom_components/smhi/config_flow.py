...
async def _validate_input(hass, latitude: float, longitude: float) -> dict[str, Any]:
    ...
    except (TimeoutError, asyncio.TimeoutError, ServerTimeoutError, ClientConnectorError, OSError) as err:
        raise ConnectionError(ERROR_CANNOT_CONNECT) from err
    ...
    return {"created_time": payload.get("createdTime"), "reference_time": payload.get("referenceTime"), "grid_point": grid_point}
...
def _schema(hass, defaults: dict[str, Any] | None = None) -> vol.Schema:
    ...
    return vol.Schema({
        ...
        vol.Optional(CONF_LATITUDE, default=defaults.get(CONF_LATITUDE, round(hass.config.latitude, 6))): selector.NumberSelector(selector.NumberSelectorConfig(min=-90, max=90, step=0.000001, mode=selector.NumberSelectorMode.BOX)),
        vol.Optional(CONF_LONGITUDE, default=defaults.get(CONF_LONGITUDE, round(hass.config.longitude, 6))): selector.NumberSelector(selector.NumberSelectorConfig(min=-180, max=180, step=0.000001, mode=selector.NumberSelectorMode.BOX)),
    })
...
class SmhiConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            ...
            try:
                ...
                self._validated_data = {CONF_NAME: name, CONF_USE_HOME_LOCATION: use_home, CONF_LATITUDE: latitude, CONF_LONGITUDE: longitude}
                return await self.async_step_confirm()
        return self.async_show_form(
            step_id="user",
            data_schema=_schema(self.hass, user_input),
            errors=errors,
            description_placeholders={"home_latitude": f"{self.hass.config.latitude:.6f}", "home_longitude": f"{self.hass.config.longitude:.6f}"},
        )
    ...
    async def async_step_confirm(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            return self.async_create_entry(title=self._validated_data[CONF_NAME], data=self._validated_data)
        return self.async_show_form(
            step_id="confirm",
...
class SmhiOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None):
        ...
        return self.async_show_form(step_id="init", data_schema=vol.Schema({
            vol.Optional(CONF_FORECAST_TIMESERIES, default=self.entry.options.get(CONF_FORECAST_TIMESERIES, DEFAULT_FORECAST_TIMESERIES)): selector.NumberSelector(selector.NumberSelectorConfig(min=1, max=200, step=1, mode=selector.NumberSelectorMode.BOX)),
            vol.Optional(CONF_SCAN_INTERVAL, default=self.entry.options.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL_MIN)): selector.NumberSelector(selector.NumberSelectorConfig(min=5, max=180, step=1, mode=selector.NumberSelectorMode.BOX)),
            ...
            vol.Optional(CONF_ENABLE_THERMAL_SENSORS, default=self.entry.options.get(CONF_ENABLE_THERMAL_SENSORS, False)): bool,
        }))
